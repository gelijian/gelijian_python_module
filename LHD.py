# Copyright (C) 2018, Gelijian @ Peking University
# for calculating the WF of the TOF spectrometer at LHD
...
_torus = Torus(R=3.75*meters, k=2.0)
...
def _coordinatesfile(file):
    print('Start to read coordinates file')
    ...
    for i, dt in enumerate(data):
        if i+1001 < len(data):
            x = i/1000
            y = (dt+data[i+1000])/2
            result.append(tuple([(float(x)+0.5)/29, 0, y[0], y[1]]))
        else:
            break
        return result
...
class _Coordinates:
    def __init__(self, file):
        ...
        self.data = _coordinatesfile(file)
        log.info('processing...')
...
class _ThermalProfile:
    ...
    def density(self, r, theta, phi):
        ...
        if V is not None:
            x = V[0]
            return bell(self.n_core, x, alpha=8.0, beta=1.0, a=0.95)
        else:
            return None
    ...
    def Te(self, r, theta, phi):
        if self.Te_core:
            ...
            if V is not None:
                x = V[0]
                return bell(self.Te_core, x, alpha=2.0, beta=1.0, a=0.95)
            else:
                return None
        else:
            return None
    ...
    def Ti(self, r, theta, phi):
        ...
        if V is not None:
            x = V[0]
            return bell(self.Ti_core, x, alpha=2.0, beta=1.0, a=0.95)
        else:
            return None
...
class _MonoBeamProfile:
    ...
    def density(self, r, theta, phi):
        ...
        if V is not None:
            x = V[0]
            return rectangle(self.n_core, x, rho_max=0.8)
        else:
            return None
